...
# ── Config ───────────────────────────────────────────────────────────────────
SHOTS = 256
# 8 delays: covers the critical t > 60us discriminating regime
DELAY_US = [0, 10, 30, 60, 100, 160, 260, 400]
# 3 bases sufficient for qubit-0 Bloch vector: ZZ→rz, XZ→rx, YZ→ry
TOMO_BASES = ['ZZ', 'XZ', 'YZ']
# Total: 8 delays × 3 bases × 256 shots = 24 circuits, ~6k shots
...
def bloch_entropy_fast(probs_zz, probs_xz, probs_yz):
    """
    Compute S_A for qubit 0 via Bloch vector from 3 measurement bases.
    ZZ -> rz, XZ -> rx, YZ -> ry  (Qiskit bit order: bits[1] = qubit 0).
    """
    def ev0(probs):
        e = 0.0
        for bits, p in probs.items():
            ...
            e += s * p
        return e
    rx = ev0(probs_xz); ry = ev0(probs_yz); rz = ev0(probs_zz)
    r  = np.clip(np.sqrt(rx**2 + ry**2 + rz**2), 0.0, 1.0)
...
def analyse_results(labels, job_results, delays_us):
    ...
    for label, qr in zip(labels, job_results):
        ...
        tomo_data[delay][basis] = {k: v/total for k, v in counts.items()}

    # Compute S_A at each delay via fast Bloch path
    t_vals, SA_vals = [], []
    for delay in sorted(tomo_data.keys()):
        d = tomo_data[delay]
        SA = bloch_entropy_fast(d.get('ZZ',{}), d.get('XZ',{}), d.get('YZ',{}))
        t_vals.append(delay)
        SA_vals.append(SA)
