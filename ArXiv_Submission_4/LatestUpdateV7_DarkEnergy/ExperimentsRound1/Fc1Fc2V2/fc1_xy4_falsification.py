...
NUM_QUBITS   = 2
# Minimal Bloch-vector bases: ZZ→rz, XZ→rx, YZ→ry on qubit 0.
# 3 bases instead of 9 gives identical S_A with 3× fewer circuits.
TOMO_BASES   = ['ZZ', 'XZ', 'YZ']
# Total: 4 delays × 2 conditions × 3 bases × 256 shots = 24 circuits, ~6k shots
# Estimated runtime on Heron r2: ~60-80 seconds
...
# ── Analysis ──────────────────────────────────────────────────────────────────
def bloch_entropy(probs_zz, probs_xz, probs_yz):
    """
    Compute S_A for qubit 0 via Bloch vector from 3 measurement bases.
    ZZ -> rz, XZ -> rx, YZ -> ry  (qubit 0 is bits[1] in Qiskit bit order).
    """
    def ev0(probs):
        e = 0.0
        for bits, p in probs.items():
            s = +1 if bits[1] == '0' else -1
            e += s * p
        return e
    rx, ry, rz = ev0(probs_xz), ev0(probs_yz), ev0(probs_zz)
    r  = np.clip(np.sqrt(rx**2 + ry**2 + rz**2), 0, 1)
...
def analyse_results(labels, job_results, shots=SHOTS):
    """
    Parse job results, compute S_A via 3-basis Bloch vector,
    ...
    for label, qr in zip(labels, job_results):
        ...
        tomo_data[(condition, delay)][basis] = {k: v/total for k,v in counts.items()}

    results_out = {}
    for delay in sorted(set(d for _, d in tomo_data.keys())):
        bare = tomo_data.get(('bare', delay), {})
        xy4  = tomo_data.get(('xy4',  delay), {})
        if not bare or not xy4:
            continue

        SA_bare = bloch_entropy(bare.get('ZZ',{}), bare.get('XZ',{}), bare.get('YZ',{}))
        SA_xy4  = bloch_entropy(xy4.get('ZZ',{}),  xy4.get('XZ',{}),  xy4.get('YZ',{}))
        ...
        status    = ("✓ CONFIRMED" if delta_pct <  5 else
    ...
    if results_out:
        ...
        print("\n" + "="*68)
        if max_delta < 5:
            print(f"VERDICT: THEOREM 1 CONFIRMED  (max Δ = {max_delta:.1f}% < 5%)")
        elif max_delta > 20:
